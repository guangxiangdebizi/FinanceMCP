...
export function formatDividend(data: any[]): string {
  ...
  for (const item of data) {
    ...
    if (item.stk_div) output += `每股送转: ${item.stk_div}股\n`;
    if (item.stk_bo_rate) output += `每股送股: ${item.stk_bo_rate}股\n`;
    if (item.stk_co_rate) output += `每股转增: ${item.stk_co_rate}股\n`;
    if (item.cash_div) output += `每股分红（税后）: ${item.cash_div}元\n`;
    if (item.cash_div_tax) output += `每股分红（税前）: ${item.cash_div_tax}元\n`;
