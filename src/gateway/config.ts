...
const PROTOCOLS = new Set<GatewayProtocol>(['openai-responses', 'openai-chat', 'anthropic']);
const CACHE_PROVIDERS = new Set<CacheProvider>(['openai', 'anthropic', 'deepseek', 'generic', 'none']);
...
function readConfigFile(path: string): GatewayFileConfig {
  let parsed: unknown;
  try {
  ...
  const config = parsed as Partial<GatewayFileConfig>;
  if (config.version !== 1) throw new Error('CACHE_GATEWAY_CONFIG version must be 1');
  return config as GatewayFileConfig;
}

function validateClient(client: GatewayClientConfig, index: number): GatewayClientConfig {
...
function validateRoute(route: GatewayModelRoute, index: number): GatewayModelRoute {
  if (!route.id?.trim()) throw new Error(`models[${index}].id is required`);
  if (!route.upstream?.baseUrl?.trim()) throw new Error(`models[${index}].upstream.baseUrl is required`);
...
function envClient(): GatewayClientConfig[] {
...
function envRoute(): GatewayModelRoute[] {
  ...
  if (!PROTOCOLS.has(rawProtocol as GatewayProtocol)) {
  ...
  if (!CACHE_PROVIDERS.has(rawProvider as CacheProvider)) {
  ...
  return [validateRoute({
    ...
    upstream: {
      protocol: rawProtocol as GatewayProtocol,
      baseUrl,
      model,
      apiKeyEnv: 'CACHE_UPSTREAM_API_KEY',
    },
    cache: {
      provider: rawProvider as CacheProvider,
      ...
      ttl: process.env.CACHE_NATIVE_CACHE_TTL as '5m' | '1h' | '30m' | undefined,
    },
  }, 0)];
}

export function loadGatewayConfig(): ResolvedGatewayConfig {
  ...
  for (const route of models) {
    for (const name of [route.id, ...(route.aliases ?? [])]) {
      ...
      if (routeNames.has(normalized)) throw new Error(`Duplicate model route or alias: ${name}`);
  ...
  const allowAnonymous = parseBoolean(
    process.env.CACHE_GATEWAY_ALLOW_ANONYMOUS,
    file?.allowAnonymous ?? false
  );
...
export function resolveModelRoute(config: ResolvedGatewayConfig, requestedModel: string): GatewayModelRoute | undefined {
  const target = requestedModel.trim().toLowerCase();
  return config.models.find(route =>
    route.id.toLowerCase() === target || (route.aliases ?? []).some(alias => alias.toLowerCase() === target)
  );
}
