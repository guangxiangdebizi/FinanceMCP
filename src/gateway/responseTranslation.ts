...
import { CanonicalResponse, CanonicalToolCall, GatewayProtocol } from './types.js';
import { isRecord } from './utils.js';
import { parseCanonicalResponse } from './canonical.js';

function createdUnix(): number {
...
function openAiFinishReason(response: CanonicalResponse): string {
  if (response.stopReason === 'tool_calls') return 'tool_calls';
  if (response.stopReason === 'length') return 'length';
  if (response.stopReason === 'content_filter') return 'content_filter';
  return 'stop';
}

function anthropicStopReason(response: CanonicalResponse): string {
  if (response.stopReason === 'tool_calls') return 'tool_use';
  if (response.stopReason === 'length') return 'max_tokens';
  return 'end_turn';
}

export function renderCanonicalResponse(
...
function renderChatStream(response: CanonicalResponse, publicModel: string): string {
...
function renderResponsesStream(response: CanonicalResponse, publicModel: string): string {
...
function renderAnthropicStream(response: CanonicalResponse, publicModel: string): string {
...
export function renderCanonicalStream(
  protocol: GatewayProtocol,
  response: CanonicalResponse,
  publicModel: string
): string {
  if (protocol === 'openai-chat') return renderChatStream(response, publicModel);
  if (protocol === 'openai-responses') return renderResponsesStream(response, publicModel);
...
function parseSseEvents(value: string): ParsedSseEvent[] {
...
function aggregateChatStream(events: ParsedSseEvent[]): CanonicalResponse | undefined {
  let id = '';
  let model = '';
  let text = '';
  let finish: unknown;
  ...
  if (!id && !text && !calls.size) return undefined;
...
function aggregateResponsesStream(events: ParsedSseEvent[]): CanonicalResponse | undefined {
  let id = '';
  let model = '';
  let text = '';
  let usage: Record<string, unknown> = {};
  ...
  for (const event of events) {
    ...
    if (payload.type === 'response.created' && isRecord(payload.response)) {
      ...
      if (existing && typeof payload.delta === 'string') existing.arguments += payload.delta;
    }
    if (isRecord(payload.usage)) usage = payload.usage;
  }
  if (!id && !text && !calls.size) return undefined;
...
function aggregateAnthropicStream(events: ParsedSseEvent[]): CanonicalResponse | undefined {
  let id = '';
  let model = '';
  let text = '';
  ...
  for (const event of events) {
    ...
    if (payload.type === 'message_start' && isRecord(payload.message)) {
      ...
      if (payload.delta.type === 'text_delta' && typeof payload.delta.text === 'string') text += payload.delta.text;
      ...
      if (isRecord(payload.delta)) stopReason = payload.delta.stop_reason;
      if (isRecord(payload.usage)) outputTokens = Number(payload.usage.output_tokens ?? outputTokens);
    }
  }
  if (!id && !text && !blocks.size) return undefined;
  const content: Record<string, unknown>[] = [];
  if (text) content.push({ type: 'text', text });
  for (const block of blocks.values()) {
    if (block.type !== 'tool_use') continue;
    let input: unknown = {};
    try { input = JSON.parse(block.json || '{}'); } catch { input = block.json; }
...
export function parseCanonicalStream(protocol: GatewayProtocol, value: string): CanonicalResponse | undefined {
  const events = parseSseEvents(value);
  if (protocol === 'openai-chat') return aggregateChatStream(events);
  if (protocol === 'openai-responses') return aggregateResponsesStream(events);
