...
export interface AppliedCachePolicy {
  provider: string;
  mode: string;
  routingKeyApplied: boolean;
  breakpointApplied: boolean;
}

function markLastTextBlock(
  content: unknown,
  blockType: 'text' | 'input_text' = 'text'
  ...
  if (!Array.isArray(content)) return { value: content, marked: false };

  const copy = content.map(item => isRecord(item) ? { ...item } : item);
  for (let index = copy.length - 1; index >= 0; index -= 1) {
    const block = copy[index];
    if (!isRecord(block) || !['text', 'input_text'].includes(String(block.type))) continue;
...
function addOpenAiExplicitBreakpoint(body: Record<string, unknown>, protocol: GatewayProtocol): boolean {
  if (protocol === 'openai-chat' && Array.isArray(body.messages)) {
    for (let index = body.messages.length - 1; index >= 0; index -= 1) {
      const message = body.messages[index];
      if (!isRecord(message) || !['system', 'developer'].includes(String(message.role))) continue;
  ...
  if (protocol === 'openai-responses') {
    if (Array.isArray(body.input)) {
      for (let index = body.input.length - 1; index >= 0; index -= 1) {
        ...
        if (!isRecord(item) || !['system', 'developer'].includes(String(item.role))) continue;
...
export function applyNativeCachePolicy(
  body: Record<string, unknown>,
  targetProtocol: GatewayProtocol,
  route: GatewayModelRoute,
  providerRoutingKey: string
): AppliedCachePolicy {
...
export function buildModelFingerprint(route: GatewayModelRoute, upstreamApiKey: string): string {
