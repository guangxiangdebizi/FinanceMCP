...
export const futuresData = {
    ...
    async run(args) {
        try {
            ...
            const params = { trade_date: args.trade_date };
            if (args.symbol)
                params.symbol = args.symbol;
            if (args.broker)
                params.broker = args.broker;
            const { data } = await callTushare('fut_holding', params, 'trade_date,symbol,broker,long_hld,long_chg,short_hld,short_chg');
            if (!data.length) {
...
function formatFuturesHolding(data, tradeDate, symbol) {
