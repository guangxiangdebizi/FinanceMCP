import { callQverisCapability, createQverisSessionId, discoverQverisCapabilities, inspectQverisCapabilities, } from '../utils/qverisClient.js';
...
function compactSearch(response) {
    return {
        source: 'Qveris',
...
function compactExecution(response) {
    return {
        source: 'Qveris',
        ...response,
    };
...
export const qverisFinance = {
    name: 'qveris_finance',
    description: '可选 Qveris 金融能力入口。先 discover 搜索行情、基本面、财务、宏观固收、新闻舆情、加密和另类数据；' +
    ...
    async run(args) {
        ...
        if (args.action === 'discover') {
            ...
            if (!query)
                throw new Error('discover 必须提供非空 query');
        ...
        if (args.action === 'inspect') {
            ...
            if (toolIds.length === 0)
                throw new Error('inspect 必须提供 tool_ids');
            if (toolIds.length > 20)
                throw new Error('inspect 单次最多检查 20 个 tool_id');
        ...
        if (args.action === 'call') {
            ...
            if (!toolId)
                throw new Error('call 必须提供 discover/inspect 返回的完整 tool_id');
            if (!searchId)
                throw new Error('call 必须提供返回该 tool_id 的 search_id');
