...
import { INDEX_DATA_TYPES, InvalidToolInputError, normalizeFinancialCode, normalizeOptionalDate, normalizeOptionalEnumInput, } from '../utils/inputValidation.js';
    ...
    async run(args) {
        try {
            ...
            if (args.data_type === 'valuation') {
                ...
                const { data, fields } = await callTushare('index_dailybasic', {
                    ts_code: args.code,
                ...
                if (!data.length)
                    throw new Error(`未找到指数${args.code}的估值数据`);
                ...
                text += `|------|-----------|------------|--------|------------|----|---------|----|  \n`;
                sorted.forEach(r => {
                    const mv = (v) => v != null ? (parseFloat(v) / 1e8).toFixed(2) : 'N/A';
                    const n2 = (v) => v != null ? parseFloat(v).toFixed(2) : 'N/A';
            ...
            if (args.data_type === 'weekly' || args.data_type === 'monthly' || args.data_type === 'global') {
                ...
                const { data } = await callTushare(config.apiName, {
                    ts_code: args.code,
                    start_date: args.start_date || defaultStart,
                    end_date: args.end_date || defaultEnd,
                }, config.fields);
                if (!data.length)
                    throw new Error(`未找到指数${args.code}的${config.title}数据`);
...
function getIndexHistoryConfig(dataType) {
    if (dataType === 'weekly') {
        return {
...
function formatIndexHistory(data, code, title, apiName) {
    const sorted = [...data].sort((a, b) => (b.trade_date || '').localeCompare(a.trade_date || ''));
