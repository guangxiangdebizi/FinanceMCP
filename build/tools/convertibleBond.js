    ...
    async run(args) {
        try {
            ...
            if (args.data_type === 'rate') {
                if (!args.ts_code)
                    throw new Error('data_type=rate 需要提供 ts_code（可转债代码）');
                const { data } = await callTushare('cb_rate', { ts_code: args.ts_code }, 'ts_code,rate_freq,coupon_rate,rate_start_date,rate_end_date');
                if (!data.length)
                    throw new Error(`未找到 ${args.ts_code} 的票面利率数据`);
                return { content: [{ type: 'text', text: formatCbRate(data, args.ts_code) }] };
            }
            if (args.data_type === 'rating') {
                if (!args.ts_code)
                    throw new Error('data_type=rating 需要提供 ts_code（可转债代码）');
                const { data } = await callTushare('cb_rating', { ts_code: args.ts_code }, 'ts_code,ann_date,rating_date,rating_com_name,rating_way,rating_type,rating,rating_outlook');
                if (!data.length)
                    throw new Error(`未找到 ${args.ts_code} 的评级数据`);
                return { content: [{ type: 'text', text: formatCbRating(data, args.ts_code) }] };
            }
            if (args.data_type === 'holders') {
                if (!args.ts_code)
                    throw new Error('data_type=holders 需要提供 ts_code（可转债代码）');
                const params = { ts_code: args.ts_code };
                if (args.period) {
                ...
                const { data } = await callTushare('top10_cb_holders', params, 'ts_code,end_date,holder_rank,holder_name,hold_amount,hold_ratio');
                if (!data.length)
                    throw new Error(`未找到 ${args.ts_code} 的十大持有人数据`);
...
function formatCbRate(data, tsCode) {
...
function formatCbRating(data, tsCode) {
    ...
    out += `|--------|--------|----------|----------|----------|------|------|\n`;
    sorted.forEach(r => {
        const n = (v) => v !== undefined && v !== null && v !== '' ? String(v) : 'N/A';
...
function formatCbHolders(data, tsCode) {
