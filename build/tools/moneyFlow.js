    ...
    async run(args) {
        try {
            ...
            if (queryType === 'hsgt_flow') {
                ...
                if (!data.length)
                    throw new Error('未找到沪深港通资金流数据，请确认日期参数');
            ...
            if (queryType === 'stock_ths') {
                ...
                if (!data.length)
                    throw new Error('未找到同花顺个股资金流数据，请确认股票代码或日期参数');
            ...
            if (queryType === 'concept_ths') {
                ...
                if (!data.length)
                    throw new Error('未找到同花顺概念资金流数据，请确认概念代码或日期参数');
...
// ── V3: 沪深港通与同花顺资金流 ─────────────────────────────────────────────
async function fetchHsgtMoneyFlow(tradeDate, startDate, endDate) {
    const params = {};
    if (tradeDate) {
        params.trade_date = tradeDate;
    }
    ...
    return await callTushare('moneyflow_hsgt', params, 'trade_date,ggt_ss,ggt_sz,hgt,sgt,north_money,south_money');
}
async function fetchThsStockMoneyFlow(tsCode, tradeDate, startDate, endDate) {
    const params = {};
    if (tsCode)
    ...
    return await callTushare('moneyflow_ths', params, 'trade_date,ts_code,name,pct_change,latest,net_amount,net_d5_amount,buy_lg_amount,buy_lg_amount_rate,buy_md_amount,buy_md_amount_rate,buy_sm_amount,buy_sm_amount_rate');
}
async function fetchThsConceptMoneyFlow(tsCode, tradeDate, startDate, endDate) {
    const params = {};
    if (tsCode)
        params.ts_code = tsCode;
    if (tradeDate) {
    ...
    return await callTushare('moneyflow_cnt_ths', params, 'trade_date,ts_code,name,lead_stock,close_price,pct_change,industry_index,company_num,pct_change_stock,net_buy_amount,net_sell_amount,net_amount');
}
function formatHsgtMoneyFlow(data) {
...
function formatThsStockMoneyFlow(data, tsCode) {
    const sorted = [...data].sort((a, b) => (b.trade_date || '').localeCompare(a.trade_date || ''));
...
function formatThsConceptMoneyFlow(data, tsCode) {
    const sorted = [...data].sort((a, b) => (b.trade_date || '').localeCompare(a.trade_date || ''));
