...
function contentBlocks(value, unsupported) {
    if (typeof value === 'string')
...
function parseChatMessages(value, unsupported) {
    if (!Array.isArray(value))
        return [];
    const messages = [];
    for (const item of value) {
        ...
        const role = ['system', 'developer', 'user', 'assistant', 'tool'].includes(rawRole)
            ? rawRole
            : 'user';
...
function parseResponsesInput(value, unsupported) {
    if (typeof value === 'string')
    ...
    for (const item of value) {
        ...
        if (item.type === 'message' || typeof item.role === 'string') {
            const rawRole = asString(item.role) ?? 'user';
            const role = ['system', 'developer', 'user', 'assistant'].includes(rawRole)
                ? rawRole
                : 'user';
...
function parseSystem(value, unsupported) {
    const blocks = contentBlocks(value, unsupported);
    return blocks.length ? [{ role: 'system', content: blocks }] : [];
}
export function parseCanonicalRequest(protocol, body) {
    const unsupported = new Set();
    const requestedModel = asString(body.model)?.trim();
    if (!requestedModel)
        throw new Error('Request body must include a non-empty model');
    let messages;
...
function ensureTranslatable(request) {
...
function renderToolChoice(source, target, value) {
    if (value === undefined)
        return undefined;
    if (target === 'anthropic') {
        if (typeof value === 'string') {
...
function renderChatMessages(messages) {
    ...
    for (const message of messages) {
        ...
        const rendered = { role: message.role, content: text || null };
        if (message.name)
...
function renderResponsesInput(messages) {
    const result = [];
    for (const message of messages) {
        const text = textFromBlocks(message.content);
        if (text)
            result.push({ role: message.role === 'tool' ? 'user' : message.role, content: text });
...
function appendAnthropicMessage(result, role, content) {
    if (!content.length)
        return;
...
export function renderCanonicalRequest(request, targetProtocol, upstreamModel, defaultMaxOutputTokens) {
...
export function injectHandoffIntoBody(body, protocol, handoffText) {
...
export function canonicalPrefixDigest(request, modelFingerprint) {
...
export function canonicalRequestDigest(request, modelFingerprint) {
    return sha256(stableJson({ modelFingerprint, request }));
}
export function canonicalMessageDigests(messages) {
    return messages.map(message => sha256(stableJson(message)));
}
export function visibleStoredMessages(messages) {
    ...
    return messages.flatMap(message => {
        ...
        if (!text || message.role === 'system' || message.role === 'developer')
            return [];
        return [{
                role: message.role,
                text,
...
export function responseStoredMessages(response) {
    const parts = [];
    if (response.text.trim())
        parts.push(response.text.trim());
    if (response.toolCalls.length) {
        parts.push(response.toolCalls.map(call => `[tool_call ${call.name} id=${call.id}] ${typeof call.arguments === 'string' ? call.arguments : JSON.stringify(call.arguments)}`).join('\n'));
    }
    if (!parts.length)
        return [];
    const text = parts.join('\n');
...
function normalizeStopReason(value) {
    if (['tool_calls', 'tool_use'].includes(String(value)))
        return 'tool_calls';
    if (['length', 'max_tokens'].includes(String(value)))
        return 'length';
    if (String(value) === 'content_filter')
        return 'content_filter';
    if (['stop', 'end_turn', 'completed'].includes(String(value)))
        return 'stop';
    return 'unknown';
}
...
function parseResponsesResponse(payload) {
    const textParts = [];
    const toolCalls = [];
...
export function parseCanonicalResponse(protocol, payload) {
    if (!isRecord(payload))
