...
import { canonicalPrefixDigest, canonicalRequestDigest, cloneRequestBody, injectHandoffIntoBody, parseCanonicalRequest, parseCanonicalResponse, renderCanonicalRequest, UnsupportedGatewayFeatureError, } from './canonical.js';
...
import { parseCanonicalStream, renderCanonicalResponse, renderCanonicalStream, } from './responseTranslation.js';
...
function explicitContextId(req, body) {
...
function bodyWithoutGatewayMetadata(body) {
    const cloned = cloneRequestBody(body);
    if (!isRecord(cloned.metadata) || !Object.hasOwn(cloned.metadata, 'fmc_context_id'))
        return cloned;
...
function requiredHandoffPolicy(route) {
    return {
...
function copySafeUpstreamHeaders(upstream, res) {
...
function setGatewayHeaders(res, contextId, matchMode, handoffInjected, cacheProvider, cacheMode) {
...
function upstreamBody(inboundProtocol, route, bodyWithHandoff, canonicalWithHandoff) {
    ...
    return {
        body: renderCanonicalRequest(canonicalWithHandoff, route.upstream.protocol, upstreamModel, route.defaultMaxOutputTokens ?? 4096),
        translated: true,
    };
}
async function upstreamError(upstream) {
    ...
    if (!text)
        return `Upstream HTTP ${upstream.status}`;
    try {
...
async function streamAndCapture(upstream, res, protocol, maxCaptureBytes = 8 * 1024 * 1024) {
    if (!upstream.body) {
        return undefined;
    }
...
function protocolError(res, protocol, status, message) {
...
async function handleInference(req, res, config, store) {
    ...
    const appliedPolicy = applyNativeCachePolicy(prepared.body, route.upstream.protocol, route, providerCacheKey);
    ...
    setGatewayHeaders(res, resolution.context.id, resolution.matchMode, resolution.handoffInjected, appliedPolicy.provider, appliedPolicy.mode);
    if (prepared.translated && requestedStream)
        res.setHeader('X-FMC-Stream-Mode', 'buffered-translation');
    ...
    try {
        ...
        if (!prepared.translated && requestedStream) {
            ...
            if (prepared.translated) {
                if (requestedStream) {
                    ...
                    res.status(200).json(renderCanonicalResponse(inboundProtocol, canonicalResponse, canonicalOriginal.requestedModel));
                }
            }
            else {
        ...
        if (!prepared.translated && requestedStream && !res.writableEnded)
            res.end();
    }
    catch (error) {
...
function identityFor(req, config) {
